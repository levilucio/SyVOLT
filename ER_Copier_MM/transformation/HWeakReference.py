...
class HWeakReference(Himesis):
    def __init__(self):
        """
        Creates the himesis graph representing the DSLTrans rule WeakReference.
        """
        # Flag this instance as compiled now
        self.is_compiled = True
        
        super(HWeakReference, self).__init__(name='HWeakReference', num_nodes=0, edges=[])
...
a.""")
        ...
        self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'WeakReference')
        
        # match model. We only support one match model
        self.add_node()
        self.vs[0]["mm__"] = """MatchModel"""
        #self.vs[0]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'WeakReferencematchmodel0')
        
        # apply model node
        self.add_node()
        self.vs[1]["mm__"] = """ApplyModel"""
        #self.vs[1]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'WeakReferenceapplymodel1')
        
        # paired with relation between match and apply models
        self.add_node()
        self.vs[2]["mm__"] = """paired_with"""
        #self.vs[2]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'WeakReferencepairedwith2')
        ...
        self.add_edges([
    		...
    		(6,5), # apply_contains -> apply_class WeakReference()
    		(3,7), # match_class WeakReference() -> has_match_attribute name ()
    		(7,8), #  has_match_attribute name () -> match_attribute name ()
    		(5,9), # apply_class WeakReference() -> has_apply_attribute name ()
    		...
    		(5,14), # apply_class WeakReference() -> has_apply_attribute ApplyAttribute ()
