...
class HERCopierProp_CompleteLHS(HimesisPreConditionPatternLHS):
        def __init__(self):
                """
                Creates the himesis graph representing the AToM3 model HERCopierProp_CompleteLHS.
                """
                # Flag this instance as compiled now
                self.is_compiled = True

                super(HERCopierProp_CompleteLHS, self).__init__(name='HERCopierProp_CompleteLHS', num_nodes=0, edges=[])
...
"""
                ...
                # Add the edges
                self.add_edges([
                (3,10), # apply_class Feature() -> backward_association
                (10,0), #  backward_association -> apply_class Feature()
                (4,11), # apply_class EntityType() -> backward_association
                (11,1), #  backward_association -> apply_class EntityType()
                (5,12), # apply_class ERModel() -> backward_association
                (12,2), #  backward_association -> apply_class ERModel()
                (4,8), # apply_class EntityType() -> association features
                (8,3), # association features  -> apply_class Feature()
                (5,9), # apply_class ERModel() -> association entities
                (9,4), # association entities  -> apply_class EntityType()
                (2,6), # match_class ERModel() -> association entities
                (6,1), # association entities  -> match_class EntityType()
                (1,7), # match_class EntityType() -> association features
                (7,0) # association features  -> match_class Feature()
                ])
