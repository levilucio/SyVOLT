...
class Disambiguator():
    ...
    def __init__(self, verbosity):
        ...
        try:
            #print("Old superclasses: " + str(match.condition["superclasses_dict"]))
            del match.condition["superclasses_dict"]["Member"]
            #print("New superclasses: " + str(match.condition["superclasses_dict"]))
    ...
    def set_contract(self, contract):
        ...
        self.contract_mms = []
        mms = contract.complete.vs["mm__"]
        superclasses = contract.complete["superclasses_dict"]

        for mm in set(mms):

            mm = mm.replace("MT_pre__", "")
            self.contract_mms.append(mm)

            if mm in superclasses:
                self.contract_mms += superclasses[mm]

        self.contract_mms = sorted(self.contract_mms)

        #print("Contract MMs: " + str(self.contract_mms))

        new_contract_mms = []

        saved_superclasses = self.find_elements_collapse_match.condition["superclasses_dict"]

        for mm in self.contract_mms:
            if mm not in saved_superclasses.keys() or 'MetaModelElement_S' not in saved_superclasses[mm]:
                continue
            new_contract_mms.append(mm)

        self.contract_mms = new_contract_mms

        return self.contract_mms
