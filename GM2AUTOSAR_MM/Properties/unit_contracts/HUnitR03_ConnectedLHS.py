...
class HUnitR03_ConnectedLHS(HimesisPreConditionPatternLHS):
	def __init__(self):
		"""
		Creates the himesis graph representing the AToM3 model HUnitR03_ConnectedLHS
		"""
		# Flag this instance as compiled now
		self.is_compiled = True

		super(HUnitR03_ConnectedLHS, self).__init__(name='HUnitR03_ConnectedLHS', num_nodes=0, edges=[])
		...
		self["name"] = """"""
		self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'HUnitR03_ConnectedLHS')
		self["equations"] = []
		...
		self.vs[3]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'3.0.m.0PhysicalNodeassoc33.0.m.1Partition')
		...
		self.vs[4]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'3.0.m.1Partitionassoc43.0.m.2Module')

		# Add the edges
		self.add_edges([
			(0,3), # match class PhysicalNode(3.0.m.0PhysicalNode) -> association partition
			(3,1), # association Partition -> match class Partition(3.0.m.1Partition)
			(1,4), # match class Partition(3.0.m.1Partition) -> association module
			(4,2), # association Module -> match class Module(3.0.m.2Module)
		])
