...
class HUnitR04a_ConnectedLHS(HimesisPreConditionPatternLHS):
	def __init__(self):
		"""
		Creates the himesis graph representing the AToM3 model HUnitR04a_ConnectedLHS
		"""
		# Flag this instance as compiled now
		self.is_compiled = True

		super(HUnitR04a_ConnectedLHS, self).__init__(name='HUnitR04a_ConnectedLHS', num_nodes=0, edges=[])
		...
		self["name"] = """"""
		self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'HUnitR04a_ConnectedLHS')
		self["equations"] = []
		...
		self.vs[5]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'4.0.m.0PhysicalNodeassoc54.0.m.1Partition')
		...
		self.vs[6]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'4.0.m.1Partitionassoc64.0.m.2Module')
		...
		self.vs[7]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'4.0.m.2Moduleassoc74.0.m.3Scheduler')

		# match association Scheduler--provided-->Servicenode
		self.add_node()
		self.vs[8]["MT_pre__attr1"] = """return attr_value == "provided" """
		self.vs[8]["MT_label__"] = """9"""
		self.vs[8]["mm__"] = """MT_pre__directLink_S"""
		self.vs[8]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'4.0.m.3Schedulerassoc84.0.m.4Service')

		# Add the edges
		self.add_edges([
			(0,5), # match class PhysicalNode(4.0.m.0PhysicalNode) -> association partition
			(5,1), # association Partition -> match class Partition(4.0.m.1Partition)
			(1,6), # match class Partition(4.0.m.1Partition) -> association module
			(6,2), # association Module -> match class Module(4.0.m.2Module)
			(2,7), # match class Module(4.0.m.2Module) -> association scheduler
			(7,3), # association Scheduler -> match class Scheduler(4.0.m.3Scheduler)
			(3,8), # match class Scheduler(4.0.m.3Scheduler) -> association provided
			(8,4), # association Service -> match class Service(4.0.m.4Service)
		])
