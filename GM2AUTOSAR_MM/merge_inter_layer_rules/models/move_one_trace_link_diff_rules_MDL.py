"""
__move_one_trace_link_diff_rules_MDL.py_____________________________________________________

Automatically generated AToM3 Model File (Do not modify directly)
Author: levi
Modified: Fri Sep  6 17:15:40 2013
...
def move_one_trace_link_diff_rules_MDL(self, rootNode, MT_pre__GM2AUTOSAR_MMRootNode=None, MT_post__GM2AUTOSAR_MMRootNode=None, MoTifRuleRootNode=None):
    ...
    if( MoTifRuleRootNode ): 
        ...
        # name
        MoTifRuleRootNode.name.setValue('MoveOneTraceLinkDiffRules')
    # --- ASG attributes over ---
    ...
    if self.genGraphics:
       new_obj = graph_MT_pre__MetaModelElement_S(500.0,424.0,self.obj71)
    ...
    if self.genGraphics:
       new_obj = graph_MT_pre__MetaModelElement_S(580.0,244.0,self.obj72)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("MT_pre__MetaModelElement_S", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
       new_obj.layConstraints['scale'] = [1.0, 1.0]
    ...
    if self.genGraphics:
       new_obj = graph_MT_pre__MetaModelElement_T(700.0,424.0,self.obj73)
       new_obj.DrawObject(self.UMLmodel)
    ...
    if self.genGraphics:
       new_obj = graph_MT_pre__trace_link(809.0,407.5,self.obj74)
       new_obj.DrawObject(self.UMLmodel)
    ...
    if self.genGraphics:
       new_obj = graph_MT_post__MetaModelElement_S(1020.0,424.0,self.obj75)
       new_obj.DrawObject(self.UMLmodel)
    ...
    if self.genGraphics:
       new_obj = graph_MT_post__MetaModelElement_S(1113.0,243.0,self.obj76)
       new_obj.DrawObject(self.UMLmodel)
       self.UMLmodel.addtag_withtag("MT_post__MetaModelElement_S", new_obj.tag)
       new_obj.layConstraints = dict() # Graphical Layout Constraints 
       new_obj.layConstraints['scale'] = [1.0, 1.0]
    ...
    if self.genGraphics:
       new_obj = graph_MT_post__MetaModelElement_T(1233.0,423.0,self.obj77)
       new_obj.DrawObject(self.UMLmodel)
    ...
    if self.genGraphics:
       new_obj = graph_MT_post__trace_link(1295.5,497.0,self.obj78)
       new_obj.DrawObject(self.UMLmodel)
    ...
    if self.genGraphics:
       new_obj = graph_RHS(920.0,184.0,self.obj79)
       new_obj.DrawObject(self.UMLmodel)
    ...
    # constraint
    self.obj80.constraint.setValue('#if len([i for i in graph.neighbors(PreNode(\'9\').index) if graph.vs[i][\'mm__\'] == \'backward_link\']) == 0:\n#    return True\n\n#return False\nreturn True\n')
    self.obj80.constraint.setHeight(15)

    self.obj80.graphClass_= graph_LHS
    if self.genGraphics:
       new_obj = graph_LHS(480.0,184.0,self.obj80)
       new_obj.DrawObject(self.UMLmodel)
    ...
    self.drawConnections(
(self.obj73,self.obj74,[868.0, 497.0, 809.0, 407.5],"true", 2) )
    # Connections for obj74 (graphObject_: Obj3) of type MT_pre__trace_link
    self.drawConnections(
(self.obj74,self.obj72,[809.0, 407.5, 750.0, 318.0],"true", 2) )
    ...
    self.drawConnections(
(self.obj77,self.obj78,[1401.0, 496.0, 1295.5, 497.0],"true", 2) )
    # Connections for obj78 (graphObject_: Obj7) of type MT_post__trace_link
    self.drawConnections(
(self.obj78,self.obj75,[1295.5, 497.0, 1190.0, 498.0],"true", 2) )
...
newfunction = move_one_trace_link_diff_rules_MDL
