...
class HContractUnitR10_ConnectedLHS(HimesisPreConditionPatternLHS):
	def __init__(self, *args, **kwargs):
		"""
		Creates the himesis graph representing the AToM3 model HContractUnitR10_ConnectedLHS
		"""
		# Flag this instance as compiled now
		self.is_compiled = True

		super(HContractUnitR10_ConnectedLHS, self).__init__(name='HContractUnitR10_ConnectedLHS', num_nodes=0, edges=[])
		...
		self["name"] = """"""
		self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'HContractUnitR10_ConnectedLHS')
		self["equations"] = []
		...
		self.vs[5]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'EntityTypeassoc5Class')

		# trace association null--trace-->nullnode
		self.add_node()
		self.vs[6]["MT_label__"] = """7"""
		self.vs[6]["mm__"] = """MT_pre__trace_link"""
		self.vs[6]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'Featureassoc6Property')

		# Add the edges
		self.add_edges([
			(1,4), # match class null(Class) -> association ownedProperty
			(4,0), # association null -> match class null(Property)
			(3,5), # apply class null(Class) -> backward_association 
			(5,1), # backward_associationnull -> match_class null(Class)
			(2,6), # apply class null(Property) -> backward_association 
			(6,0), # backward_associationnull -> match_class null(Property)
		])
