...
Author: levi
Modified: Fri Dec 12 17:03:04 2014
...
def F2F_MDL(self, rootNode, PoliceStationMMRootNode=None):
    ...
    # classtype
    self.obj74.classtype.setValue('1')

    # cardinality
    self.obj74.cardinality.setValue('s_')

    # name
    self.obj74.name.setValue('s_')

    self.obj74.graphClass_= graph_Female_S
    if self.genGraphics:
       new_obj = graph_Female_S(400.0,140.0,self.obj74)
    ...
    # classtype
    self.obj75.classtype.setValue('t_')

    # name
    self.obj75.name.setValue('s_')

    self.obj75.graphClass_= graph_Female_T
    if self.genGraphics:
       new_obj = graph_Female_T(400.0,280.0,self.obj75)
       new_obj.DrawObject(self.UMLmodel)
    ...
    self.obj76=match_contains(self)
    self.obj76.isGraphObjectVisual = True

    if(hasattr(self.obj76, '_setHierarchicalLink')):
      self.obj76._setHierarchicalLink(False)

    self.obj76.graphClass_= graph_match_contains
    if self.genGraphics:
       new_obj = graph_match_contains(331.0,151.0,self.obj76)
       new_obj.DrawObject(self.UMLmodel)
    ...
    self.obj77=apply_contains(self)
    self.obj77.isGraphObjectVisual = True

    if(hasattr(self.obj77, '_setHierarchicalLink')):
      self.obj77._setHierarchicalLink(False)

    self.obj77.graphClass_= graph_apply_contains
    if self.genGraphics:
       new_obj = graph_apply_contains(333.5,351.0,self.obj77)
       new_obj.DrawObject(self.UMLmodel)
    ...
    # Connections for obj74 (graphObject_: Obj18) named s_
    self.drawConnections(
(self.obj74,self.obj78,[461.0, 181.0, 587.0, 180.5],"true", 2) )
    # Connections for obj36 (graphObject_: Obj1) of type MatchModel
    self.drawConnections(
(self.obj36,self.obj44,[201.0, 121.0, 203.5, 251.0],"true", 2),
(self.obj36,self.obj76,[201.0, 121.0, 331.0, 151.0],"true", 0) )
    # Connections for obj37 (graphObject_: Obj2) of type ApplyModel
    self.drawConnections(
(self.obj37,self.obj77,[206.0, 381.0, 333.5, 351.0],"true", 0) )
    # Connections for obj75 (graphObject_: Obj19) named s_
    self.drawConnections(
 )
...
(self.obj40,self.obj46,[909.0, 84.0, 811.0, 132.0],"true", 2) )
    ...
    # Connections for obj76 (graphObject_: Obj20) of type match_contains
    self.drawConnections(
(self.obj76,self.obj74,[331.0, 151.0, 461.0, 181.0],"true", 2) )
    # Connections for obj77 (graphObject_: Obj21) of type apply_contains
    self.drawConnections(
(self.obj77,self.obj75,[333.5, 351.0, 461.0, 321.0],"true", 2) )
    # Connections for obj44 (graphObject_: Obj9) of type paired_with
    self.drawConnections(
(self.obj44,self.obj37,[203.5, 251.0, 206.0, 381.0],"true", 2) )
    # Connections for obj78 (graphObject_: Obj22) of type hasAttr_S
    self.drawConnections(
(self.obj78,self.obj39,[587.0, 180.5, 713.0, 180.0],"true", 2) )
