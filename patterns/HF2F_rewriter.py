...
class HF2F_rewriter(HimesisPostConditionPattern):
    def __init__(self):
        """
        Creates the himesis graph representing the AToM3 model HF2F_rewriter.
        """
        # Flag this instance as compiled now
        self.is_compiled = True
        
        super(HF2F_rewriter, self).__init__(name='HF2F_rewriter', num_nodes=15, edges=[])
        
        # Add the edges
        self.add_edges([[5, 0], [0, 6], [6, 1], [1, 14], [8, 2], [2, 13], [11, 3], [3, 5], [7, 4], [4, 13], [9, 7], [8, 10], [11, 9], [10, 14], [6, 12], [12, 7]])
...
return attr_value
"""
        self.vs[12]["GUID__"] = UUID('63cfc0d5-2dc2-4215-9242-27a48a83afa0')
...
return attr_value
"""
        self.vs[13]["GUID__"] = UUID('499491f3-dfa0-4000-a1a9-47bf3acba82a')
...
return attr_value
"""
        self.vs[14]["GUID__"] = UUID('b966fe5d-3db3-4a29-8533-9248ee256139')
    ...
    def set_cardinality7(self, attr_value, PreNode, graph):
        s_
    ...
    def execute(self, packet, match):
        ...
        # Female_S7
        try:
            graph.vs[labels['7']]['cardinality'] = self.set_cardinality7(graph.vs[labels['7']]['cardinality'], lambda i: graph.vs[match[i]], graph)
        except Exception, e:
            raise Exception('An error has occurred while computing the value of the attribute \'cardinality\'', e)
        
        graph.vs[labels['7']][Himesis.Constants.MT_DIRTY] = True
        ...
        try:
            graph.vs[new_node]['classtype'] = self.set_classtype6(None, lambda i: graph.vs[match[i]], graph)
        except Exception, e:
        ...
        # Create new edges
        #===============================================================================
        # Female_T6 -> trace_linkNone
        graph.add_edges([(labels['6'], labels['None'])])
        # trace_linkNone -> Female_S7
        graph.add_edges([(labels['None'], labels['7'])])
        # ApplyModel5 -> apply_contains0
        graph.add_edges([(labels['5'], labels['0'])])
        # apply_contains0 -> Female_T6
        graph.add_edges([(labels['0'], labels['6'])])
        # Female_T6 -> hasAttr_T1
