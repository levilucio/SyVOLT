...
class HBasicStateNoOutgoing2ProcDef_rule_combinator_rewriter_1(HimesisPostConditionPattern):
    def __init__(self):
        """
        Creates the himesis graph representing the AToM3 model HBasicStateNoOutgoing2ProcDef_rule_combinator_rewriter_1.
        """
        # Flag this instance as compiled now
        self.is_compiled = True
        
        super(HBasicStateNoOutgoing2ProcDef_rule_combinator_rewriter_1, self).__init__(name='HBasicStateNoOutgoing2ProcDef_rule_combinator_rewriter_1', num_nodes=30, edges=[])
...
"""
        ...
        self.vs[14]["GUID__"] = UUID('1b40c4c3-a0cf-4d96-a4cc-6509fda67db7')
...
"""
        ...
        from HBasicStateNoOutgoing2ProcDef_rule_combinator_matcher_0 import HBasicStateNoOutgoing2ProcDef_rule_combinator_matcher_0
        self.pre = HBasicStateNoOutgoing2ProcDef_rule_combinator_matcher_0()
    ...
    def execute(self, packet, match):
        ...
        # Constant27
        try:
            graph.vs[labels['27']]['name'] = self.set_name27(graph.vs[labels['27']]['name'], lambda i: graph.vs[match[i]], graph)
        except Exception, e:
            raise Exception('An error has occurred while computing the value of the attribute \'name\'', e)
        
        graph.vs[labels['27']][Himesis.Constants.MT_DIRTY] = True
        # Constant28
        try:
            graph.vs[labels['28']]['name'] = self.set_name28(graph.vs[labels['28']]['name'], lambda i: graph.vs[match[i]], graph)
        except Exception, e:
            raise Exception('An error has occurred while computing the value of the attribute \'name\'', e)
        
        graph.vs[labels['28']][Himesis.Constants.MT_DIRTY] = True
        # Constant26
        try:
            graph.vs[labels['26']]['name'] = self.set_name26(graph.vs[labels['26']]['name'], lambda i: graph.vs[match[i]], graph)
        except Exception, e:
            raise Exception('An error has occurred while computing the value of the attribute \'name\'', e)
        
        graph.vs[labels['26']][Himesis.Constants.MT_DIRTY] = True
