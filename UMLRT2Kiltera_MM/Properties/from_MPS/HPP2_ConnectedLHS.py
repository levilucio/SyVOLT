...
class HPP2_ConnectedLHS(HimesisPreConditionPatternLHS):
	def __init__(self):
		...
		self["name"] = """"""
		self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'HPP2_ConnectedLHS')
		self["equations"] = []
		...
		self.vs[4]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'0.31.m.1Transitionassoc40.31.m.0OUT2')
		...
		self.vs[5]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'0.31.m.1Transitionassoc50.31.m.2Trigger')
		...
		self.vs[6]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'0.31.m.2Triggerassoc60.31.m.3Signal')

		# Add the edges
		self.add_edges([
			(1,4), # match class Transition(0.31.m.1Transition) -> association type
			(4,0), # association OUT2 -> match class OUT2(0.31.m.0OUT2)
			(1,5), # match class Transition(0.31.m.1Transition) -> association triggers
			(5,2), # association Trigger -> match class Trigger(0.31.m.2Trigger)
			(2,6), # match class Trigger(0.31.m.2Trigger) -> association signal
			(6,3), # association Signal -> match class Signal(0.31.m.3Signal)
		])
