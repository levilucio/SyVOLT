...
class HBasicStateNoOutgoing2ProcDef(Himesis):
    def __init__(self):
        ...
        self.add_edges([
                ...
                (1,5), # applymodel -> -> apply_class Null()
                (4,6), # apply_class ProcDef() -> association p
                (6,5), # association p  -> apply_class Null()
                (4,7), # apply_class ProcDef() -> backward_association
                (7,3), #  backward_association -> apply_class State()
                (0,2), # matchmodel -> pairedwith
                (2,1) # pairedwith -> applyModel				
		])

        # Add the attribute equations
        self["equations"] = [((3,'isComposite'),('constant','false')), ((3,'hasOutgoingTransitions'),('constant','false')), ]
