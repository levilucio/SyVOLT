...
class HUnionWomanRule(Himesis):
    def __init__(self):
        """
        Creates the himesis graph representing the DSLTrans rule UnionWomanRule.
        """
        # Flag this instance as compiled now
        self.is_compiled = True
        
        super(HUnionWomanRule, self).__init__(name='HUnionWomanRule', num_nodes=0, edges=[])
...
a.""")
        ...
        self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'UnionWomanRule')
        
        # match model. We only support one match model
        self.add_node()
        self.vs[0]["mm__"] = """MatchModel"""
        #self.vs[0]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'UnionWomanRulematchmodel0')
        
        # apply model node
        self.add_node()
        self.vs[1]["mm__"] = """ApplyModel"""
        #self.vs[1]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'UnionWomanRuleapplymodel1')
        
        # paired with relation between match and apply models
        self.add_node()
        self.vs[2]["mm__"] = """paired_with"""
        #self.vs[2]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'UnionWomanRulepairedwith2')
    	...
    	# apply attribute atom ApplyAttribute() node
    	self.add_node()
    	self.vs[30]["name"] = """famMemberWoman"""
