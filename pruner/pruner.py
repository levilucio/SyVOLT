...
class Pruner(object):
    ...
    def __init__(self, metamodel, transformation, rule_names, pc_name_function):
        ...
        for layer_num in range(len(transformation)):
            for rule in transformation[layer_num]:
                ...
                links_not_found = self.will_links_be_built(rule.name, missing_links, self.prunerHelper.ruleContainmentLinksExtended.keys(), require_all_links = True)
    ...
    def isPathConditionStillFeasible(self, pathCondition, rulesToTreat):
        ...
        missingLinks = self.will_links_be_built(pathCondition.name, pc_missing, rulesToTreat)
