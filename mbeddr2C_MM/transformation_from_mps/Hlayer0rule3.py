...
class Hlayer0rule3(Himesis):
	def __init__(self):
		...
		# match class StringType(layer0rule3class0StringType) node
		self.add_node()
		self.vs[3]["mm__"] = """StringType"""
		self.vs[3]["attr1"] = """+"""

		# apply class StringType(layer0rule3class1StringType) node
		self.add_node()
		...
		# Add the edges
		self.add_edges([
			(0,3), # matchmodel -> match_class StringType(layer0rule3class0StringType)
			(1,4), # applymodel -> apply_classStringType(layer0rule3class1StringType)
