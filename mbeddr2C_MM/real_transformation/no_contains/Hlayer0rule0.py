...
class Hlayer0rule0(Himesis):
    def __init__(self):
        ...
        # Add the edges
        self.add_edges([
                (0,3), # matchmodel -> match_class ImplementationModule(layer0rule0class0)
                (1,4), # applymodel -> -> apply_class ImplementationModule(layer0rule0class1)
                (0,2), # matchmodel -> pairedwith
                (2,1) # pairedwith -> applyModel				
		])

        # Add the attribute equations
        self["equations"] = [((4,'__ApplyAttribute'),('constant','ImplementationModule')), ((4,'name'),(3,'name')), ]
