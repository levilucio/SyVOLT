...
class HcopersonsSolveRefCountryFamilyChildCommunityWoman(Himesis):
    def __init__(self):
        ...
        """
        Creates the himesis graph representing the DSLTrans rule copersonsSolveRefCountryFamilyChildCommunityWoman.
        """
        # Flag this instance as compiled now
        self.is_compiled = True
        
        super(HcopersonsSolveRefCountryFamilyChildCommunityWoman, self).__init__(name='HcopersonsSolveRefCountryFamilyChildCommunityWoman', num_nodes=0, edges=[])
        
        
        # Set the graph attributes
        self["mm__"] = ['HimesisMM']
        self["name"] = """copersonsSolveRefCountryFamilyChildCommunityWoman"""
        self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'copersonsSolveRefCountryFamilyChildCommunityWoman')
        ...
        self.vs[2]["attr1"] = """copersonsSolveRefCountryFamilyChildCommunityWoman"""
        ...
        self.add_edges([
                ...
                (0,5), # matchmodel -> match_class Child()
                (1,6), # applymodel -> -> apply_class Community()
                (1,7), # applymodel -> -> apply_class Woman()
                (3,8), # match_class Country() -> association families
                (8,4), # association families  -> match_class Family()
                (4,9), # match_class Family() -> association daughters
                (9,5), # association daughters  -> match_class Child()
                (6,10), # apply_class Community() -> association persons
                (10,7), # association persons  -> apply_class Woman()
                (6,11), # apply_class Community() -> backward_association
                (11,3), #  backward_association -> apply_class Country()
                (7,12), # apply_class Woman() -> backward_association
                (12,5), #  backward_association -> apply_class Child()
