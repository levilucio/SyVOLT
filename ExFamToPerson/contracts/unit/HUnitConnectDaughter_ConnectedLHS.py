...
class HUnitConnectDaughter_ConnectedLHS(HimesisPreConditionPatternLHS):
	def __init__(self):
		"""
		Creates the himesis graph representing the AToM3 model HUnitConnectDaughter_ConnectedLHS
		"""
		# Flag this instance as compiled now
		self.is_compiled = True

		super(HUnitConnectDaughter_ConnectedLHS, self).__init__(name='HUnitConnectDaughter_ConnectedLHS', num_nodes=0, edges=[])
		...
		self["name"] = """"""
		self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'HUnitConnectDaughter_ConnectedLHS')
		self["equations"] = []
		...
		self.add_node()
		self.vs[4]["MT_pre__attr1"] = """return attr_value == "daughters" """
		self.vs[4]["MT_label__"] = """5"""
		self.vs[4]["mm__"] = """MT_pre__directLink_S"""
		self.vs[4]["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'9.0.m.1Familyassoc4Child')

		# Add the edges
		self.add_edges([
			(0,3), # match class Country(9.0.m.0Country) -> association families
			(3,1), # association Family -> match class Family(9.0.m.1Family)
			(1,4), # match class Family(9.0.m.1Family) -> association daughters
			(4,2), # association Parent -> match class Parent(Child)
		])
