...
class HPos_ParentCompany_CompleteLHS(HimesisPreConditionPatternLHS):
        def __init__(self, *args, **kwargs):
                """
                Creates the himesis graph representing the AToM3 model HPos_ParentCompany_CompleteLHS.
                """
                # Flag this instance as compiled now
                self.is_compiled = True

                super(HPos_ParentCompany_CompleteLHS, self).__init__(name='HPos_ParentCompany_CompleteLHS', num_nodes=0, edges=[])
...
"""
                self["name"] = """"""
                self["GUID__"] = uuid.uuid3(uuid.NAMESPACE_DNS,'Pos_ParentCompany')
...
"""
                ...
                # Add the edges
                self.add_edges([
                (3,8), # apply_class TownHall() -> backward_association
                (8,0), #  backward_association -> apply_class City()
                (4,9), # apply_class Person() -> backward_association
                (9,1), #  backward_association -> apply_class Parent()
                (3,7), # apply_class TownHall() -> association workers
